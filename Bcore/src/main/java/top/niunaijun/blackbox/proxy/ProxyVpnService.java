...
public class ProxyVpnService extends VpnService {
    ...
    // Cooldown to prevent rapid duplicate permission requests from multiple processes
    private static volatile long sLastPermissionRequestTime = 0;
    private static final long PERMISSION_REQUEST_COOLDOWN_MS = 3000; // 3 second cooldown

    // Mutex to prevent multiple threads from establishing VPN simultaneously
    private static volatile boolean sVpnEstablishing = false;
    private static final Object sVpnEstablishLock = new Object();
    ...
    protected void establishVpn() {
        // Acquire lock to prevent multiple threads from establishing simultaneously
        synchronized (sVpnEstablishLock) {
            if (sVpnEstablishing || mIsEstablished) {
                Slog.d(TAG, "VPN establishment already in progress or completed, skipping");
                return;
            }
            sVpnEstablishing = true;
        }

        try {
            // Check if we have VPN permission first
            android.content.Intent prepareIntent = VpnService.prepare(getApplicationContext());
            if (prepareIntent != null) {
                // Check cooldown to prevent duplicate requests from multiple processes
                long currentTime = System.currentTimeMillis();
                if (currentTime - sLastPermissionRequestTime < PERMISSION_REQUEST_COOLDOWN_MS) {
                    Slog.d(TAG, "VPN permission request in cooldown, skipping duplicate");
                    return;
                }
                sLastPermissionRequestTime = currentTime;

                Slog.w(TAG, "VPN permission not granted, requesting via activity");
                VpnPermissionActivity.requestVpnPermission(getApplicationContext());
                return;
            }
